      ...
      {docs.length === 0 ? (
        ...
        <>
            ...
            {docs.map((d) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                  selectedIds.includes(d.id) ? "bg-brand-deep text-white border-brand-deep" : "bg-surface text-inkmuted border-line"
                }`}
              >
                {d.filename}
              </button>
          ...
          <div className="flex gap-2 mb-6">
            ...
            />
            <button onClick={ask} disabled={asking} className="bg-brand-deep text-white px-4 py-2.5 rounded-lg flex items-center gap-1.5 text-sm font-medium disabled:opacity-60">
