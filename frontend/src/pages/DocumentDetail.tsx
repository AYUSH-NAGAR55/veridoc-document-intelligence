          ...
          {doc.status === "ready" && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 text-sm border border-line bg-surface px-3.5 py-2 rounded-lg hover:border-blue-deep/60"
              >
              ...
              {exportOpen && (
                <div className="absolute right-0 mt-1 bg-surface border border-line rounded-lg shadow-soft overflow-hidden z-10 w-40">
                  {(["xlsx", "csv", "json"] as const).map((fmt) => (
                    <a
                      key={fmt}
                      href={api.exportDocument(doc.id, fmt)}
