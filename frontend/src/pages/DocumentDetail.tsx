...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  algorithm_method: "Algorithms / Methods",
  technology_protocol_tool: "Protocols / Technologies",
  entity: "Entities",
  numerical_value: "Numerical / Factual Values",
  relationship: "Comparisons / Relationships",
  table_data: "Tables / Structured Data",
  general: "Other",
};
        ...
        <>
          ...
          {tab === "Extracted Information" && (
            <div className="bg-surface border border-line rounded-xl2 overflow-hidden">
              {!extractions ? <Skeleton className="h-40 m-5" /> : extractions.extractions.length === 0 ? (
                <EmptyState title="No structured information could be confidently extracted from this document." description="" />
              ) : extractions.verified_values.length === 0 ? (
                <EmptyState
                  title="No verified information yet."
                  description={extractions.pending_review_count > 0
                    ? `${extractions.pending_review_count} item${extractions.pending_review_count === 1 ? "" : "s"} awaiting verification.`
                    : "Items are still being processed."}
                />
              ) : (
                <div className="divide-y divide-line">
                  {extractions.pending_review_count > 0 && (
                    <p className="px-5 py-2.5 text-xs text-inkmuted bg-canvas">
                      {extractions.pending_review_count} item{extractions.pending_review_count === 1 ? "" : "s"} awaiting verification (see Review Items).
                    </p>
                  )}
                  {CATEGORY_ORDER.filter((cat) => extractions.verified_values.some((v: any) => (v.category || "general") === cat)).map((cat) => (
                    <div key={cat} className="px-5 py-4">
                      <h3 className="text-xs font-semibold uppercase text-inkmuted tracking-wide mb-3">{CATEGORY_LABELS[cat]}</h3>
                      <div className="space-y-3">
                        {extractions.verified_values
                          .filter((v: any) => (v.category || "general") === cat)
                          .map((v: any) => (
                            <div key={v.id} className="border border-line rounded-lg px-4 py-3">
                              <div className="flex items-start justify-between gap-4">
                                <div>
                                  <p className="font-medium text-ink">{v.field_name}</p>
                                  <p className="text-sm text-ink mt-0.5">{v.verified_value ?? <span className="text-inkmuted">Rejected</span>}</p>
                                </div>
                                {typeof v.confidence === "number" && <ConfidenceBadge value={v.confidence} />}
                              </div>
                              {(v.source_location || v.evidence_text) && (
                                <details className="mt-2">
                                  <summary className="text-xs text-inkmuted cursor-pointer select-none">
                                    Source: {locationLabel(v.source_location)}
                                  </summary>
                                  {v.evidence_text && <p className="text-xs text-inkmuted mt-1.5 italic">"{v.evidence_text}"</p>}
                                </details>
                              )}
                            </div>
                          ))}
