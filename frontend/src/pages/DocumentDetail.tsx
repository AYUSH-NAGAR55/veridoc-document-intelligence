    ...
    <div className="p-6 md:p-8 max-w-5xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          {doc.status === "ready" && (
            <a href={api.exportDocument(doc.id, "json")} className="flex items-center gap-1.5 text-sm border border-line bg-surface px-3.5 py-2 rounded-lg hover:border-blue-deep/60">
        ...
        <>
            ...
            {TABS.map((t) => (
              <button
                ...
                className={`whitespace-nowrap px-3.5 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
                  tab === t ? "border-blue-deep text-blue-deep" : "border-transparent text-inkmuted hover:text-ink"
                }`}
              >
                {t}
              </button>
          ...
          {tab === "Validation" && (
            <div className="space-y-3">
              {!validation ? <Skeleton className="h-24 w-full" /> : validation.map((v: any) => (
                <div key={v.id} className={`rounded-xl2 p-4 border ${v.passed === "fail" ? "bg-danger-pastel border-danger-deep/20" : v.passed === "warning" ? "bg-peach-pastel border-peach-deep/20" : "bg-mint-pastel border-mint-deep/20"}`}>
                  <p className="text-sm font-medium">{v.rule_name.replaceAll("_", " ")}</p>
                  <p className="text-sm mt-1 opacity-90">{v.message}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === "Review Items" && (
            <div className="space-y-3">
              {!reviewItems ? <Skeleton className="h-24 w-full" /> : reviewItems.length === 0 ? (
                ...
                <div key={r.id} className="bg-surface border border-line rounded-xl2 p-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-ink">{r.field_name}: {r.ai_value}</p>
                    <p className="text-xs text-inkmuted mt-1">{r.status} • {locationLabel(r.source_location)}</p>
                  </div>
                  <ConfidenceBadge value={r.confidence} />
                </div>
              ))}
              <Link to="/review-queue" className="text-sm text-blue-deep font-medium">Go to full review queue →</Link>
            </div>
          )}
          ...
          {tab === "Ask Questions" && (
            <div className="max-w-2xl">
              <div className="flex gap-2">
                ...
                />
                <button onClick={() => askQuestion(question)} disabled={asking} className="bg-blue-deep text-white text-sm font-medium px-4 py-2.5 rounded-lg disabled:opacity-60">
                  {asking ? "Asking..." : "Ask"}
                </button>
              </div>
            ...
            <div className="space-y-2">
              {!audit ? <Skeleton className="h-24 w-full" /> : audit.map((a: any) => (
                <div key={a.id} className="flex items-start gap-3 bg-surface border border-line rounded-xl2 p-4">
                  <div className="h-2 w-2 rounded-full bg-blue-deep mt-2 shrink-0" />
                  <div>
