    ...
    <div className="p-6 md:p-8 max-w-6xl">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <button
          ...
          className="bg-blue-deep text-white text-sm font-medium px-4 py-2.5 rounded-lg hover:bg-blue-deep/90 transition-colors"
        >
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 border border-line rounded-lg bg-surface text-sm focus:border-blue-deep outline-none"
          />
        </div>
        <select
          value={status}
          onChange={(e) => setStatus(e.target.value)}
          className="border border-line rounded-lg bg-surface text-sm px-3 py-2.5 focus:border-blue-deep outline-none"
        >
        ...
        <div className="bg-surface border border-line rounded-xl2">
          {search || status !== "all" ? (
            ...
            <EmptyState
              ...
              action={<button onClick={() => setShowUpload(true)} className="bg-blue-deep text-white text-sm font-medium px-4 py-2.5 rounded-lg">Upload a document</button>}
            />
          )}
        </div>
      ) : (
