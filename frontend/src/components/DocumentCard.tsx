    ...
    <div className="relative bg-surface border border-line rounded-xl2 p-5 hover:shadow-card transition-shadow">
      {onToggleSelect && (
        <input
          ...
          className="absolute top-4 right-4 h-4 w-4 accent-brand-deep"
          aria-label={`Select ${doc.filename}`}
        />
      )}
      <Link to={`/documents/${doc.id}`} className="block">
        <div className="flex items-start gap-3">
          <div className="h-10 w-10 rounded-lg bg-brand-pastel flex items-center justify-center text-brand-deep shrink-0">
            <Icon size={18} />
          </div>
