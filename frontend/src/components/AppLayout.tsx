...
import { LayoutDashboard, FileStack, ListChecks, GitCompareArrows, MessageCircleQuestion, BarChart3, Settings, ArrowLeft } from "lucide-react";
      ...
      <aside className="w-60 shrink-0 border-r border-line bg-surface hidden md:flex flex-col">
        <div className="h-16 flex items-center px-5 border-b border-line">
          <Link to="/" className="flex items-center gap-2 font-semibold text-ink">
            <span className="h-7 w-7 rounded-lg bg-blue-deep flex items-center justify-center text-white text-sm">V</span>
            VeriDoc
          </Link>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  ...
                  isActive ? "bg-blue-pastel text-blue-deep font-medium" : "text-inkmuted hover:bg-canvas hover:text-ink"
                )
              }
            >
              <Icon size={17} />
        ...
        <div className="md:hidden h-14 border-b border-line bg-surface flex items-center px-4 gap-3 overflow-x-auto sticky top-0 z-30">
          {NAV_ITEMS.map(({ to, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                clsx("text-xs whitespace-nowrap px-2.5 py-1.5 rounded-full", isActive ? "bg-blue-pastel text-blue-deep font-medium" : "text-inkmuted")
              }
            >
              {label}
            </NavLink>
