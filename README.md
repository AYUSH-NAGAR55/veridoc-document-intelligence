...
Edit `.env`:
- Default is `LLM_PROVIDER=ollama` — install [Ollama](https://ollama.com), run `ollama pull llama3.1` (or your preferred model), and make sure `ollama serve` is running before starting the backend. Extraction and Q&A then run entirely locally, no API key, no data leaves your machine.
- Set `LLM_PROVIDER=openai` and `LLM_API_KEY=sk-...` to use a real hosted model instead, **or**
- Set `LLM_PROVIDER=mock` to try VeriDoc fully offline with no LLM at all (deterministic demo output, useful for quick UI testing without installing anything).
...
| `LLM_PROVIDER` | `ollama` \| `openai` \| `mock` | `ollama` |
| `LLM_API_KEY` / `LLM_API_BASE` / `LLM_MODEL` | OpenAI-compatible provider config | — |
| `OLLAMA_BASE_URL` / `OLLAMA_MODEL` | Local Ollama config | — |
| `EMBEDDING_BACKEND` | `lightweight` (no extra deps, low memory) \| `sentence-transformers` (better quality, needs `requirements-embeddings.txt` + ~1GB RAM) | `lightweight` |
| `EMBEDDING_MODEL` | Sentence-Transformers model name (only used if backend is `sentence-transformers`) | `all-MiniLM-L6-v2` |
...
- **"Document gets stuck on 'Indexing' forever"** — this is almost always an
  out-of-memory kill on a constrained host (e.g. Render's free 512MB tier),
  usually caused by `EMBEDDING_BACKEND=sentence-transformers` loading PyTorch.
  Check your platform's logs around the time of upload for `Killed` or a
  silent restart. Fix: leave `EMBEDDING_BACKEND=lightweight` (the default) on
  free/small tiers - it needs no heavy ML dependencies. Only switch to
  `sentence-transformers` on a host with ~1GB+ RAM available.
- **"Could not reach Ollama at http://localhost:11434"** — Ollama isn't
  running, or the model isn't pulled yet. Run `ollama serve` (often starts
  automatically after installing), then `ollama pull llama3.1` (or whichever
  model you set in `OLLAMA_MODEL`). Answers will fall back to "Not found in
  the uploaded document" while Ollama is unreachable, rather than crashing.
