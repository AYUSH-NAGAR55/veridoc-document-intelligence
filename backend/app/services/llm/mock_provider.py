...
class MockProvider(LLMProvider):
    ...
    # -- RAG answer: find the retrieved lines with the strongest keyword
    #    overlap, and only answer if that overlap clears a real relevance
    #    bar. Confidence scales with how much of the question was actually
    #    matched, instead of a fixed number - a weak match should not look
    #    as sure as a strong one. --
    def _answer(self, prompt: str) -> str:
        ...
        for line in context.splitlines():
            content = re.sub(r"^\[\d+\]\s*\([^)]*\)\s*", "", line).strip()  # strip "[1] (source label) "
            if not content:
                continue
            words = {w.lower() for w in re.findall(r"\w+", content)}
            overlap = q_words & words
            if not overlap:
                continue
            candidates.append({"text": content, "score": len(overlap), "word_count": len(content.split())})

        if not candidates:
            return json.dumps({"answer": None, "confidence": 0.0, "found": False})

        top_score = max(c["score"] for c in candidates)
        overlap_ratio = top_score / len(q_words)
        if overlap_ratio < MIN_QUESTION_OVERLAP_RATIO:
            return json.dumps({"answer": None, "confidence": 0.0, "found": False})

        # A short heading/title line ties on raw keyword-overlap count with a
        # full explanatory sentence just as easily (both contain the same 2-4
        # matched words; extra prose in the sentence doesn't reduce the
        # intersection size). Previously the FIRST line to reach the top
        # score won - and headings are typically the first line of a
        # page/slide's extracted text, so the heading almost always won.
        # Instead, among tied top matches, prefer the most substantive
        # (longest) line(s), and combine up to two of them, so an offline
        # "answer" is a real sentence rather than a bare heading.
        tied = [c for c in candidates if c["score"] == top_score]
        # If any tied match is a real sentence (not just a short heading),
        # use only substantive matches - a heading tacked onto the end of a
        # real sentence adds noise, not information. Only fall back to short
        # matches if nothing substantive tied for the top score.
        substantive = [c for c in tied if c["word_count"] >= 6]
        finalists = sorted(substantive or tied, key=lambda c: c["word_count"], reverse=True)[:2]
        answer_text = " ".join(c["text"] for c in finalists)[:400]

        confidence = round(min(0.9, 0.35 + overlap_ratio * 0.55), 2)
        return json.dumps({"answer": answer_text, "confidence": confidence, "found": True})
