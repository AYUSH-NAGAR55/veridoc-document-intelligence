...
def answer_question(db: Session, question: str, document_ids: list[str] | None = None, top_k: int = 5) -> dict:
    ...
    # Only the strongest few retrieved chunks are used - passing every retrieved
    # chunk to the model (and showing it to the user as "evidence") produces
    # noisy, low-relevance citations for anything beyond the real match.
    ...
    # Show only the top 2 sources that genuinely back the answer, not every chunk retrieved.
    sources = [{
