...
def run_pipeline(document_id: str):
    db = SessionLocal()
    try:
        ...
        try:
            _process_understanding(db, document)
...
def _process_understanding(db: Session, document: Document):
    ...
    dup_doc, score = find_duplicate(db, hash_file(file_bytes), full_text)
