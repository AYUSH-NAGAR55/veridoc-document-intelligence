...
@router.get("")
def list_review_queue(db: Session = Depends(get_db), status: str = "pending"):
...
def _get_item(db: Session, item_id: str) -> ReviewItem:
...
def accept_item(item_id: str, payload: ReviewActionRequest, db: Session = Depends(get_db)):
...
def correct_item(item_id: str, payload: ReviewCorrectionRequest, db: Session = Depends(get_db)):
    item = _get_item(db, item_id)
...
def reject_item(item_id: str, payload: ReviewActionRequest, db: Session = Depends(get_db)):
    item = _get_item(db, item_id)
